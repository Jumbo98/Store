using Catalog as service from '../../srv/store.cds';
...
annotate service.ProductsView with {
    stock @(Common.FieldControl: #Mandatory,
    )
};
...
annotate service.CompanyManufacturersView with @(UI.PresentationVariant #vh_ProductsView_company: {
    $Type    : 'UI.PresentationVariantType',
    SortOrder: [{
...
annotate service.ProductsView with {
    onlyFreshProducts @(Common.Label: 'Только свежие продукты');
};
...
annotate service.OrderItemsView with @(UI.LineItem #Itemsoforder: [
    {
        $Type: 'UI.DataField',
        Value: product_ID,
        Label: '{i18n>Product}',
...
annotate service.FreshProducts with @(UI.PresentationVariant #vh_OrderItemsView_product: {
    $Type    : 'UI.PresentationVariantType',
    SortOrder: [{
        $Type     : 'Common.SortOrderType',
        Property  : expireDate,
